...
if __name__ == "__main__":
	...
	writer = csv.writer(f)
	writer.writerow( ('ref', 'title', 'authors', 'l_authors', 'abstract', 'species', 'latlong', 'iconName', 'year', 'type', 'secondary', 'keywords', 'l_key', 'circuit') )
 ...
 #    f.close()
	...
	# table = {'cognitive':{'ref':'NUMBER', 'title': 'STRING', 'authors':'STRING', 'l_authors':'STRING', 'abstract':'STRING', 'species':'STRING', 'latlong':'LOCATION', 'iconName':'STRING', 'year':'STRING', 'type':'STRING', 'secondary':'STRING', 'keywords':'STRING', 'l_key':'STRING', 'circuit':'STRING'}}
	# tableid = int(ft_client.query(SQL().createTable(table)).split("\n")[1])
	# print 'Reference Table: %s' % (tableid)
	...
	for key in combined_db.keys():
		...
		# print latlong
		authors = ' '.join(tref['en_authors'])
		abstract = tref['pm_abstract'].encode('utf-8')
		species = tref['species'].encode('utf-8')
		title = tref['en_title'][0].encode('utf-8')

		circuit = tref['circuit'].encode('utf-8')
		

		year = tref['en_year'][0].encode('utf-8')
		reftype = '('+tref['en_type']+')'
		# print reftype

		if len(tref['en_secondary']) > 0:
			secondary = tref['en_secondary'][0]
		else:
			secondary = ''
